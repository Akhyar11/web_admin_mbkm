...
export default function Pengurus() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(assets.API + "/pengurus");
      setData(response.data.pengurus);
    } catch (err) {
      console.log(err);
    }
  };
    ...
    <div className="w-full flex flex-col gap-5">
      ...
      <div className="w-full flex flex-col gap-4 bg-primery rounded-lg p-6">
        <div className="flex justify-between">
          <span className="block text-lg font-semibold">Pengurus</span>
        ...
        <div>
          <div className="flex justify-between w-full border-b py-2">
            ...
            <p className="block w-1/4">Jabatan</p>
          ...
          {data.map((i) => {
            return (
              ...
              >
                ...
                />
                <p className="block w-1/4">{i.nama}</p>
                <p className="block w-1/4">{i.jabatan}</p>
                <div className="w-1/4 overflow-auto flex lg:flex-row flex-col gap-2 lg:gap-5">
                  <Link
                    href={"/pengurus/edit/" + i.id}
